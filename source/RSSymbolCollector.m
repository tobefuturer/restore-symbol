...
static inline uint8 n_sectForAddress(uint64 address, NSArray *commands);
- (BOOL)generateAppendStringTable:(NSData **)stringTable appendSymbolTable:(NSData **)symbolTable {
    if (_symbols.count == 0) {
        *stringTable = [NSData new];
        *symbolTable = [NSData new];
        return NO;
    }
    ...
    for (int i = 0; i < _symbols.count; i ++) {
        ...
        if (is32Bit) {
            ...
            list[i].n_sect = n_sectForAddress(symbol.address, _machOFile.loadCommands);
...
static inline uint8 n_sectForAddress(uint64 address, NSArray *commands)
{
    ...
    for (id loadCommand in commands) {
